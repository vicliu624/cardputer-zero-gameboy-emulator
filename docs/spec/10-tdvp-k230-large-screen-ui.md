...
| Physical presentation | The user-visible UI coordinate system is 1232x568 landscape. Labwc owns the verified RM69A10 connector's 568x1232 native DRM scanout and panel transform; SDL supplies a normal Wayland window and input while the application presents CPU-owned XRGB `wl_shm` buffers directly to it. The game rectangle is copied at 3x nearest-neighbour scaling to 720x480, and static UI pixels are cached between state changes. |
